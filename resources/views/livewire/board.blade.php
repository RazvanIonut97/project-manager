<div class="p-4 select-none font-semibold ">
    <div class="h-12 w-full flex justify-between px-3 items-center mb-3 bg-gray-200 rounded ">
        <div class="flex">
            ...
            @endif
            <h3>{{$project->name}}-{{$selectedTask}}</h3>
        </div>

        @can('edit', $project)
        <livewire:add-list :project="$project->id">
        <livewire:history-select :history="$project->history_status" :project="$project->id">
    ...
    <div class="bg-gray-200 min-h-full scrollbar scrollbar-thumb-gray-900 scrollbar-track-gray-100 overflow-x-scroll ">
        <div wire:sortable="updateGroupOrder" 
        wire:sortable-group="updateTaskOrder" 
        class="flex px-4 py-6 items-start ">
            ...
            <div wire:key="group-{{ $group->id }}" wire:sortable.item="{{ $group->id }}"
                ...
                <div class="flex justify-between p-2  rounded-t-lg bg-gray-800 text-white">
                    ...
                    @can('edit', $project)
                    <button
                        onclick='Livewire.emit("openModal", "delete-group-modal",{{ json_encode(["group" => $group->id]) }})'
